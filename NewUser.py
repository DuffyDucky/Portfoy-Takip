...
def check_password():
    """Kullanıcı doğru şifreyi girerse True döner."""
    # Şifre daha önce girildiyse kontrol etme
    ...
    def password_entered():
        if st.session_state["password"] == st.secrets["password"]:
            ...
            del st.session_state["password"]  # Şifreyi güvenlik için sil
        else:
            st.session_state["password_correct"] = False

    # Şifre girilmediyse kutucuğu göster
    st.text_input(
        "🔒 Lütfen Erişim Şifresini Girin:", type="password", on_change=password_entered, key="password"
    )
